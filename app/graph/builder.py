...
from app.graph.nodes.retrieve_bcm_context import make_retrieve_bcm_context_node
...
def build_graph(
    ...
    # Fan-out after embedding: retrieve test-case candidates AND BCM spec context in parallel
    graph.add_node("retrieve_candidates",  make_retrieve_candidates_node(qdrant_client))
    graph.add_node("retrieve_bcm_context", make_retrieve_bcm_context_node(bcm_client))
    ...
    graph.add_edge("embed_requirement", "retrieve_bcm_context")

    # Fan-in: score_coverage waits for both branches
    graph.add_edge("retrieve_candidates",  "score_coverage")
    graph.add_edge("retrieve_bcm_context", "score_coverage")
